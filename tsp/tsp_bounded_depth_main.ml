(** The main program for bounded depth search in TSP.
...
let parse_args () =
  ...
  in
	...
	"--memory", Arg.String (fun s -> ()), "ignored";
...
(*
	  Verb.pr Verb.toplvl "Solution: %s\n"
	    (Tsp_bounded_depth.string_of_node p);
*)
	  Tsp.check_tour prob.dists (List.rev p.part_tour) p.cost;
...
let run_tsp args limit inst =
  (** [run_tsp alg limit ints] runs the algorithm on the
      instance using TSP. *)
  let module I = Bounded_depth_interface in
  let alg = Tsp_bounded_depth_algs.by_args args in
  let init, inter = Tsp_bounded_depth.make_interface inst in
  let make_interface _ = init, inter in
  let log_incumbent = make_incumbent_logger inter.I.leaf_cost in
    output inst inter.I.leaf_cost
...
let main () =
  ...
  let inst = Tsp_instances.read_tsplib stdin
  in Verb.with_level verb (fun () -> run_tsp args limit inst)
