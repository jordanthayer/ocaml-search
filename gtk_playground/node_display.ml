(* Handles display of node information, in its own window *)
...
let data_lbls =
  ...
  [|GMisc.label ~text:"" ~packing:(val_table#attach ~left:1 ~top:0) ();
    ...
    GMisc.label ~text:"" ~packing:(val_table#attach ~left:1 ~top:3) ();
    GMisc.label ~text:"" ~packing:(val_table#attach ~left:1 ~top:4) ();
    GMisc.label ~text:"" ~packing:(val_table#attach ~left:1 ~top:5) ();
    GMisc.label ~text:"" ~packing:(val_table#attach ~left:1 ~top:6) ();
    GMisc.label ~text:"" ~packing:(val_table#attach ~left:1 ~top:7) ();
    GMisc.label ~text:"" ~packing:(val_table#attach ~left:1 ~top:8) ();
    GMisc.label ~text:"" ~packing:(val_table#attach ~left:1 ~top:9) ();
    GMisc.label ~text:"" ~packing:(val_table#attach ~left:1 ~top:10) ();|]

(* Uncomment this section once you move it back into search vis *)
(*
...
let display_node key_printer exp getters step =
  ...
  do
    (let fn,nm = getters.(i) in
	   ...
	   Render.Scaler v ->
	     data_lbls.(i+1)#set_text (string_of_float v)
	 | Render.Fixed (r,g,b) -> ())
  done
*)
