...
(********************** informal testing *****************************)
let ipath = "./grid/data/instance/uniform"
...
let u8compare_algs ?(w = 70) ?(h = 20) ?(p = 0.3) ?(seed = 31415) algs =
  let b = Math.with_random_seed (fun () -> unit_eight_board w h p) seed in
    compare_algs_on algs b

let compare_algs ?(w = 70) ?(h = 20) ?(p = 0.3) ?(seed = 31415) algs =
  let b = Math.with_random_seed (fun () -> life_four_board w h p) seed in
    compare_algs_on algs b

let compare_algs_unit ?(w = 70) ?(h = 20) ?(p = 0.3) ?(seed = 31415) algs =
  let b = Math.with_random_seed (fun () -> unit_four_board w h p) seed in
    compare_algs_on algs b
...
let compare_algs_u_random ?(w = 70) ?(h = 20) ?(p = 0.3) () =
  compare_algs_u (life_four_board w h p)
...
let test_heuristic ?(w = 70) ?(h = 20) n =
  List.iter
    (fun costs ->
	  ...
	  let b = (feasible_board w h 0. costs ways) in
	    Wrutils.pr "done.\n%!";
	    Wrutils.ntimes
	      (fun () ->
		 let b = { b with
