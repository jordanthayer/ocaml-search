...
let make_get_node board bound f_q geq i =
  let get_node () =
    ...
    then (incr on_dhat;
	  let x,y = best_d.data.Grid.pos in
	    !board.(x).(y) <- 1.::!board.(x).(y);
	    best_d)
    else (if best_fh.fp.est_f <= wf
      then (incr on_fhat;
	    let x,y = best_fh.data.Grid.pos in
	    !board.(x).(y) <- 2.::!board.(x).(y);
	    best_fh)
      else (incr on_f;
	    let x,y = best_f.data.Grid.pos in
...
let search board i key hash equals goal expand initial bound =
  ...
  let get_node = make_get_node board bound clean openlist i in
...
let dups board sface args =
  ...
  reset();
    let i = search board i key hash equals goal expand initial bound in
