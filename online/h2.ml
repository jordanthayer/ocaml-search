...
let init_reg_set atoms acts =
  let atoms = Wrlist.remove_dups Wrutils.identity atoms in
...
let regress_i_j (i,j) ai aj =
  ...
  else let s = Wrlist.remove_dups Wrutils.identity
...
let init_pairs pairs atoms achievers =
  (** build the pairs structure; preparing for DP procedure *)
...
let h2_reg p =
  ...
  start_h2 ();
  let nF = List.length p.atoms
  and init = List.filter p.in_initial p.atoms in
