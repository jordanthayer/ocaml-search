...
let advance s =
  (** advance state [s] in the forward direction to the earliest ending
    time (from init) of any action in s.in_progress *)
  let delta_c =
  ...
  in
	...
	(let delta = delta_c in
	 let done_a_c,in_progress = List.partition (fun (_,d) -> d = delta)
				      s.in_progress
	 and done_a_p,prev_plan = fst (List.hd s.prev_plan),(List.tl s.prev_plan) in
...
let select_applicables p state =
  ...
  let acts = List.fold_left (fun prev atom ->
		(filter_applicables state (p.supportees atom) prev) @ prev)
	       [] state.current in
    Verb.pe 5 "Applicable actions: %d \n%s\n" (List.length acts)
  ...
  (fun s ->
	 ...
	 List.map (fun a ->
		     if (s.plan = []) && (s.in_progress = []) then
			   ...
			   current = Wrlist.subtractq s.current a.delete;
			   in_progress = (a,a.dur)::s.in_progress;})
	   (select_applicables p s) in
	 if (List.length s.in_progress = 0) && (List.length s.prev_plan = 0) then
	   (Verb.pe 5 "%d children (no Advance)\n%!" (List.length children);
	    children)
	 else
...
let make_rp p =
  ...
  Cg.distance_p p (Rtpg_c.closest_p p)
...
let make_state_h h =
  ...
  (fun s ->
     let h_val = (float) (snd (h s.current s.in_progress)) in
