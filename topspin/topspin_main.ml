(**
   main program for the topspin solver.
...
let parse_non_alg_args () =
  ...
  let parse_memory_limit mls = 
	...
	"--heuristic", Arg.Set_string heuristic_name,
	"Choice of heuristic to use";
...
let main () =
  ...
  let v, limit, heuristic_name, args = 
    parse_non_alg_args () in
    Verb.with_level v
      (fun () ->
	 let inst = Topspin_io.read_problem stdin in
	 let iface = (Topspin_interfaces.topspin_interface heuristic_name) in
	 let alg = set_up_alg iface inst args in
	   run_alg Topspin.sol_length alg inst limit
      )
