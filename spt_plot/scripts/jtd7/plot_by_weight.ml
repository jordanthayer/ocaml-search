...
let xy_plot ?(xkey="wt") ?(xlabel = "Suboptimality Bound")
...
let xy_plot_norm ?(xkey="wt") ?(xlabel = "Suboptimality Bound")
    ~title ~ylabel datasets ykey =
...
let tiles ?(ykey = "total nodes generated") ?(xmin = 1.1) ?(xmax = 5.)
    alg_list =
  let dat = List.map
...
let contract_tiles ?(ykey = "total nodes generated") ?(xmin = 1.1) ?(xmax = 5.)
    alg_list =
  let dat = List.map
    (Jtd7_helpers.load_wrap (Load_dataset.tiles))
    alg_list in
  let dat = Dataset_macros.filter_all_unsolved dat in
  let dat = List.map (Dataset.filter float_of_string
			(fun v -> v <= xmax && v >= xmin) "contract") dat in
    xy_plot ~title:"Korf's 100 15 Puzzles" ~ylabel:ykey dat ykey
...
let sqrt_tiles ?(ykey = "total nodes generated") ?(xmin = 1.1) ?(xmax = 5.)
    alg_list =
  let dat = List.map
    (Jtd7_helpers.load_wrap (Load_dataset.korf_100_tiles ~cost:"sqrt"))
    alg_list in
  let dat = List.map (Dataset.filter float_of_string
			(fun v -> v <= xmax && v >= xmin) "wt") dat in
    xy_plot ~title:"Korf's 100 15 Puzzles" ~ylabel:ykey dat ykey
...
let inv_tiles ?(ykey = "total nodes generated") ?(xmin = 1.1) ?(xmax = 5.)
    alg_list =
  let dat = List.map
...
let u8grid ?(ykey = "total nodes generated") alg_list =
  let dat = List.map
    (Jtd7_helpers.load_wrap Load_dataset.standard_unit_8_grids) alg_list in
    xy_plot ~title:"Unit Eight-way Grid World" ~ylabel:ykey dat ykey


let l4grid ?(ykey = "total nodes generated") ?(xmin = 1.) ?(xmax = 5.)alg_list =
  let dat = List.map
...
let l4grid_spech ?(ykey = "total nodes generated") ?(xmin = 1.) ?(xmax = 5.)
    alg_list =
  let dat = List.map
    (Jtd7_helpers.load_wrap Load_dataset.standard_life_4_grids_noh) alg_list in
  let dat = List.map (Dataset.filter float_of_string
			(fun v -> v >= xmin && v <= xmax) "wt") dat in
    xy_plot ~title:"Life Four-way Grid World" ~ylabel:ykey dat ykey


let corrupt_l4grid ?(ykey = "total nodes generated")
    ?(xmin = 0.) ?(xmax = 1.) alg_list =
  let dat = List.map
    (Jtd7_helpers.load_wrap Load_dataset.corrupt_life_4_grids) alg_list in
    xy_plot ~xkey:"wt" ~title:"Life Four-way Grid World"
      ~xlabel:"Suboptimality Bound"
...
let l8grid ?(ykey = "total nodes generated") alg_list =
  let dat = List.map
    (Jtd7_helpers.load_wrap Load_dataset.standard_life_8_grids) alg_list in
    xy_plot ~title:"Life Eight-way Grid World" ~ylabel:ykey dat ykey
...
let pkhard ?(ykey = "total nodes generated") ?(xmax = 3.)
    ?(xmin = 1.1) alg_list =
  let dat = List.map
    (Jtd7_helpers.load_wrap Load_dataset.standard_pkhard) alg_list in
  let dat = List.map (Dataset.filter float_of_string
			(fun v -> v <= xmax && v >= xmin) "wt") dat in
    xy_plot ~title:"100 City TSP - PK Hard" ~ylabel:ykey dat ykey


let usquare ?(ykey = "total nodes generated") ?(xmax = 3.)
    ?(xmin = 1.1) alg_list =
  let dat = List.map
    (Jtd7_helpers.load_wrap Load_dataset.standard_usquare) alg_list in
  let dat = List.map (Dataset.filter float_of_string
			(fun v -> v <= xmax && v >= xmin) "wt") dat in
    xy_plot ~title:"100 City TSP - Unit Square" ~ylabel:ykey dat ykey


let vacuum_noh ?(ykey = "total nodes generated") alg_list =
  let dat = List.map
    (Jtd7_helpers.load_wrap Load_dataset.standard_vacuum) alg_list in
  let dat = List.map (Dataset.filter float_of_string
			(fun v -> v >= 1.5) "wt") dat in
    xy_plot ~title:"Vacuum World" ~ylabel:ykey dat ykey


let small_vacuum ?(ykey = "total nodes generated") ?(xmin = 1.) ?(xmax = 5.)
    alg_list =
  let dat = List.map
    (Jtd7_helpers.load_wrap Load_dataset.small_vacuum) alg_list in
  let dat = List.map (Dataset.filter float_of_string
			(fun v -> v >= xmin && v <= xmax) "wt") dat in
    xy_plot ~title:"Vacuum World" ~ylabel:ykey dat ykey


let vacuum_chris ?(ykey = "total nodes generated") alg_list =
  let dat = List.map
    (Jtd7_helpers.load_wrap Load_dataset.chris_standard_vacuum) alg_list in
  let dat = List.map (Dataset.filter float_of_string
			(fun v -> v >= 1.75) "wt") dat in
    xy_plot ~title:"Vacuum World" ~ylabel:ykey dat ykey


let vacuum_standard ?(ykey = "total nodes generated") alg_list =
  let dat = List.map
...
let heavy_vacuum ?(ykey = "total nodes generated") ?(xmin = 1.5) ?(xmax = 5.)
    alg_list =
  let dat = List.map
...
let solved_heavy_vacuum ?(ykey = "total nodes generated") ?(xmin = 1.5) ?(xmax = 5.)
    alg_list =
  let dat = List.map
    (Jtd7_helpers.load_wrap Load_dataset.heavy_vacuum) alg_list in
  let dat = List.map (Dataset.filter float_of_string
...
let vacuum_greedy ?(ykey = "total nodes generated") alg_list =
  let dat = List.map
    (Jtd7_helpers.load_wrap Load_dataset.greedy_standard_vacuum) alg_list in
  let dat = List.map (Dataset.filter float_of_string
			(fun v -> v >= 1.5) "wt") dat in
    xy_plot  ~title:"Vacuum World" ~ylabel:ykey dat ykey
...
let robots ?(ykey = "total nodes generated") ?(xmax = 5.) alg_list =
  let dat = List.map
...
let dock_robot ?(ykey = "total raw cpu time") ?(xmin = 1.2) ?(xmax = 5.)
    alg_list =
  let dat = List.map
...
let big_dock_robot ?(ykey = "total raw cpu time") ?(xmin = 1.2) ?(xmax = 5.)
    alg_list =
  let dat = List.map
...
let plot_aggregate ?(ykey = "final sol cost")
...
let plot_aggregate_old ?(ykey = "final sol cost")
    talgs italgs l4algs vacalgs robalgs docalgs =
  let tdat = List.map (Jtd7_helpers.load_wrap
			 (Load_dataset.korf_100_tiles ~cost:"unit")) talgs in
    Verb.pe Verb.always "Tiles loaded!\n%!";
  ...
  in Verb.pe Verb.always "Dynamic Robot Loaded!\n%!";
  let doc = List.map
    (Jtd7_helpers.load_wrap Load_dataset.dock_robot) docalgs in
    Verb.pe Verb.always "All Datasets loaded\n%!";
  let agg_lst = [tdat; itdat; l4g; vac; rob; doc;] in
  let agg = List.fold_left (fun accum el ->
			      List.map2 (fun a b -> Dataset.merge [a;b])
...
let dock_robot_nc ?(ykey = "final sol cost") ?(xmin = 1.2) ?(xmax = 5.)
    alg_list =
  let dat = List.map
    (Jtd7_helpers.load_wrap Load_dataset.big_dock_robot) alg_list in
  let dat = List.map (Dataset.filter float_of_string
