...
let rec expand_prime ?(record = no_record) pq i sface =
  ...
  then
    (record i pq pq;
	 ...
	 (Limit.incr_prune i;
	  expand_prime ~record:record pq i sface)
...
let rec expand_prime_dups ?(record = no_record) pq i sface closed better_p
    get_p_pos =
  (** Expands nodes from the optimistic queue until an incumbent solution
  ...
  let consider_child n =
    ...
    then
	 ...
	 if not (Limit.promising_p i n) then
	   (Limit.incr_prune i;
	    expand_prime_dups ~record:record pq i sface closed better_p get_p_pos)
...
let rec expand_prime_delay_dups ?(record = no_record) pq i sface closed dpq
  ...
  let consider_child n =
	...
	let n = Dpq.extract_first pq in
	 if not (Limit.promising_p i n) then
	   (Limit.incr_prune i;
	    expand_prime_delay_dups pq i sface closed dpq better_p get_p_pos
	      get_d_pos)
	 else if sface.Search_interface.goal_p n then
	   (* must be new best *)
	   Limit.new_incumbent i (Limit.Incumbent (0.,n))
...
let rec expand_f ?(record = no_record) sface get_node fq pq i bound =
  ...
  then
    (record i pq fq;
     let n = get_node fq pq i bound in
       if not (Limit.promising_p i n) then
	 (Limit.incr_prune i;
	  expand_f ~record:record sface get_node fq pq i bound)
       else if sface.Search_interface.goal_p n then
	 (Limit.new_incumbent i (Limit.Incumbent (0.,n));
	  expand_f ~record:record sface get_node fq pq i bound)
       else
...
let rec expand_f_dups ?(record = no_record) sface get_node fq pq closed i bound
  ...
  and spq = Dpq.count pq in
	...
	(record i pq fq;
	 let n = get_node fq pq i bound in
	   if not (Limit.promising_p i n) then
	     (Limit.incr_prune i;
	      expand_f_dups ~record:record sface get_node fq pq closed i bound better_p
		get_p_pos get_f_pos)
	   else if sface.Search_interface.goal_p n then
	     (Limit.new_incumbent i (Limit.Incumbent (0.,n));
	      expand_f_dups ~record:record sface get_node fq pq closed i bound better_p
...
let make_delay_fqueue ordered_f set_f_pos dpq pq initial inc =
  (*make sure to only add nodes with f < incumebnt
    can cause queue inconsistencies later on.*)
  let fq = Dpq.create ordered_f set_f_pos (Dpq.count pq) initial in
...
let no_dups ?(queue_rec = no_record) sface get_node ordered_p bound better_p
    ordered_f set_pq_pos set_f_pos =
  ...
  and i = sface.Search_interface.info in
    ...
    expand_prime ~record:queue_rec pq i sface;
    (match i.Limit.incumbent with
	 Limit.Incumbent(float,n) ->
	   (let fq = make_fqueue ordered_f set_f_pos pq
		...
		expand_f ~record:queue_rec sface get_node fq pq i bound
...
let dups ?(queue_rec = no_record) sface get_node ordered_p bound better_p
  ...
  and closed = Htable.create sface.Search_interface.hash
    ...
    Dpq.insert pq sface.Search_interface.initial;
    (*Verb.pe Verb.toplvl "Beginning optimistic search\n%!";*)
    expand_prime_dups ~record:queue_rec pq i sface closed better_p get_p_pos;
    (match i.Limit.incumbent with
	 Limit.Incumbent(float,n) ->
	   (let fq = make_fqueue ordered_f set_f_pos pq
	      sface.Search_interface.initial i in
	      try
		(*Verb.pe Verb.toplvl "cleaningingup\n%!";*)
		expand_f_dups ~record:queue_rec sface get_node fq pq closed i bound better_p
		  get_p_pos get_f_pos
	      with Done -> (match i.Limit.incumbent with
			       Limit.Nothing -> failwith "Done but no incumbent?"
			     | Limit.Incumbent (q,n) ->
...
let delay sface get_node ordered_p bound better_p ordered_f
  ...
  and closed = Htable.create sface.Search_interface.hash
    ...
    Htable.add closed (sface.Search_interface.key
      ...
      sface.Search_interface.initial;
    expand_prime_delay_dups pq i sface closed dpq better_p get_p_pos get_d_pos;
    (match i.Limit.incumbent with
	 Limit.Incumbent(float,n) ->
	   (let fq = make_delay_fqueue ordered_f set_f_pos dpq pq
		...
		expand_f_dups sface get_node fq pq closed i bound better_p
		  get_p_pos get_f_pos
	      with Done ->
		(match i.Limit.incumbent with
		     Limit.Nothing -> failwith "Done but no incumbent?"
