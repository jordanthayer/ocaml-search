(**
    iDTA* search algorithm as through the search interface
...
let get_samples tbl feature depth =
  (** Get a list of error samples, recreating the original data that
      created the counts table *)
  let counts =
    try
      (Hashtbl.find tbl.counts feature).(depth-1)
    with _ -> let t = Hashtbl.create 0 in
      Hashtbl.add t 0. 1; t in
    List.flatten (Wrht.map
		    (fun k v -> Array.to_list (Array.create v k)) counts)
...
let idta_learn_run ?(dups = false) orig_sface args =
  (** Perform SRTA* to gather error data for a given initial state. *)
  let horizon = Search_args.get_int "Idtastar.idta_learn" args 0 in
  let outch = open_out (Search_args.get_string "Idtastar.idta_learn" args 1) in
...
let idta_learn orig_sface args =
  Limit.unwrap_sol6_into_5 unwrap_sol
    (idta_learn_run orig_sface args ~dups:false)

let idta_learn_dups orig_sface args =
  Limit.unwrap_sol6 unwrap_sol
    (idta_learn_run orig_sface args ~dups:true)
...
let calc_scores node_structs =
  (** Samples configurations for the node error values and determines
      which node(s) are best in a given configuration, incrementing
      scores for them.*)
  (for i = 0 to (sample_size-1) do
     let best_nodes =
       fst (Wrlist.mins_by (fun ns -> ns.samples.(i)) node_structs) in
       List.iter (fun best_node -> best_node.score <- best_node.score + 2)
	 best_nodes
   done);
  node_structs


let prob_less probs info delta n d beta_g alpha_cost =
  (** Probabilty that the value of [n] will lower below [alpha_cost]
      (ie. decrease by at least [delta]) but no further than [d]
      away. *)
...
let prob_greater probs info delta n d alpha_g beta_cost =
  (** Probabilty that the value of [n] will raise above [beta_cost]
      (ie. increase by at least [delta]) but no further than [d]
      away. *)
  let accuracy = probs.accuracy in
  let trunc x = Functions.trunc_to x accuracy in
  let incr_amt = 1. /. (10.**(float accuracy)) in
  ...
  and counts = Wrht.map (fun k v -> (k, v))
    (get_counts probs n_h d) in
    (*Printf.printf "d: %d, e: %f, delta: %f, n_h: %d\n" d beta_cost
      delta n_h;*)
    if Limit.halt_p info then
	...
	let c = (List.fold_left
		   (fun acc (err_val, count) ->
			 ...
			 acc)) 0. counts) in
	  ((*Printf.printf "seen values larger than %d for %d %f times\n"
	     i (n_h+.n_g) c;*)
	    if c = 0. then
	      0.
	    else
...
let search_worthwhile openlists alpha betas horizon r info
    key probs =
	...
	((for d = 1 to horizon do
	    for i = len_m_zero to (len_alpha-1) do (* length of set m *)
	      let delta = (match get_elt () with
			     | Some delta_node ->
				 delta_node.f -. delta_node.g
			     | None -> fake_infinity) in
		((*Printf.printf "length of M-0: %d, len_alpha-1: %d\n"
		   len_m_zero (len_alpha-1);*)
		  let s_cost = (search_cost probs.branch_factor i r d) in
		    if (((prob_greater probs info delta n d
			    alpha.g beta_cost) -.
			   s_cost) > 0.) or
		      List.fold_left 
			   (fun acc b ->
			      let openlist = (Htable.find openlists
						(key b)) in
				if Dpq.empty_p openlist then false
				else 
				  let n_b = (Dpq.peek_first
					       openlist) in
				    acc or
				      (((prob_less probs info
					   (-.fake_infinity)
					   n_b d b.g alpha_cost) -.
					  s_cost) > 0.)) true betas then
	 ...
	 (Wrlist.mapi
	    (fun j act ->
		 ...
		 (Dpq.map
		    ...
		    (Htable.find table (key act)))) a)) in
    let best_struct = fst
      (Wrlist.min_by (fun ns -> abs ns.score) node_structs) in
    let rec alpha = List.nth a best_struct.q
    in
    let q = Htable.find table (key alpha)
    in
      (if (not (Dpq.empty_p q)) && (not (Limit.halt_p info)) &&
	 (e < grain_size) then
	   let n = Dpq.remove_ret q best_struct.q_index in
	     if not (Limit.promising_p i n) then
	       ((* this should never happen, since we go straight to
		   solutions in the top-level search *)
		 failwith "idta.search_step: searched after solution found";
		...
		(try
		   List.iter
			...
			Dpq.insert q n)
		     (expand n);
		 with _ -> failwith "idta.search_step");
		 ...
		 (Dpq.map
		    (fun n ->
			    ...
			    max_depth) in
			 {
			   ...
			   samples = Array.init
			     sample_size
			     (fun _ -> if (Array.length vals) == 0 then
				0.
			      else
				n.f -. (vals.(Random.int
						(Array.length vals))));
...
let idta_choose ?(record = no_record) expand key hash equals info
  ...
  let choose actions =
	...
	if ((List.length acts) == 0) then
	  ...
	  and betas = (List.tl (List.sort backup_f_then_g acts)) in
	    if goal_found then
	      ((*print_endline "found goal in idta_choose";*) List.hd acts)
	    else if Limit.halt_p info then
	      (print_endline "halt hit in idta_choose";
	       alpha)
	    else if not (search_worthwhile table
			   alpha betas horizon r info
			   key probs) then
	      ((*print_endline "search wasn't worthwhile";
		 Printf.printf "backed up f-value of alpha in idta_choose: %f\n%!" alpha.backup_f;*) alpha)
	    else
...
let idta_search ?(record = no_record) sface grain_size horizon r
    learn_path ?(online = false) dups =
  (** Uses the action chosen by idta_choose to take the best action
  ...
  let rec move curr =
	...
	ret_func i)
      ...
      let alpha = idta_choose sface.Search_interface.node_expand
...
(************ FUNCTIONS FOR CALLING THE SEARCH **********)
  ...
  let grain_size = Search_args.get_int "Idtastar.no_dups" args 0 in
  let horizon = Search_args.get_int "Idtastar.no_dups" args 1 in
  let r = Search_args.get_float "Idtastar.no_dups" args 2 in
  let learn_path = Search_args.get_string "Idtastar.dups" args 3 in
  ...
  let grain_size = Search_args.get_int "Idtastar.dups" args 0 in
  let horizon = Search_args.get_int "Idtastar.dups" args 1 in
  let r = Search_args.get_float "Idtastar.dups" args 2 in
  let learn_path = Search_args.get_string "Idtastar.dups" args 3 in
  let search_interface = make_sface sface in
    Limit.unwrap_sol6 unwrap_sol
      (idta_search
