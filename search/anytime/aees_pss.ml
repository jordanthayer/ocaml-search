...
let search i key hash equals goal expand initial b =
  ...
  let insert n =
    ...
    then (Limit.new_incumbent i (Limit.Incumbent (0., n));
	  let best_f = Dpq.peek_first !clean in
	  let prop_bound = n.fp.g /. best_f.fp.f in
	    if prop_bound < !bound
	    then (bound := prop_bound;
  ...
  let select_and_expand () =
    ...
    let wf = best_f.fp.f *. !bound in
      ...
      then (bestdh := !bestdh + 1; remove best_d; expand best_d)
      else if best_fh.fp.est_f <= wf
      then (bestfh := !bestfh + 1; remove best_fh; expand best_fh)
      else (bestf := !bestf + 1; remove best_f; expand best_f) in
  ...
  let rec do_loop () =
    ...
    do_loop ();
    Verb.pe Verb.always "bestf: %i\tbestfh: %i\tbestdh: %i\n%!"
      !bestf !bestfh !bestdh;
    i
