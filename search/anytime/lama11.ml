(**
    ...
    @author jordan
    @since 2011-11-23
    Supposed to mimic the behavior of the newest lama in domain independent planning

    Maintains two open lists, the wA* open list and the helpful action
    open list.  Will need to implement a schedule for serving from the queues eventually, but
    an initial prototype should be fine.
*)
...
let search key hash equals i root wts goal_p expand max_it nodes =
  ...
  and helplist = Dpq.create ordered_p (fun n i -> n.ha_pos <- i) 100 root
...
(*  and boost = 1000 *) in
  ...
  let insert ha n =
    ...
    if ha then Dpq.insert helplist n in


  let consider_child ha n =
    Limit.incr_gen i;
	...
	Limit.incr_dups i;
	let opos = prev.open_pos
	and hpos = prev.ha_pos in
	if not (better_p prev n)
	then (Htable.replace nodes state n;
	      if opos >= 0
	      then Dpq.remove openlist opos;
	      if hpos >= 0
	      then Dpq.remove helplist hpos;
	      insert ha n)
...
(*
  let get_node () =
    if !alt > 0 && not (Dpq.empty_p helplist)
    then (let n =  Dpq.extract_first helplist in
	  alt := !alt - 1;
	  n)
    else (alt := !alt + boost;
	  Dpq.extract_first openlist) in
*)
  let get_node () =
    if !alt && not (Dpq.empty_p helplist)
    then (let n =  Dpq.extract_first helplist in
	  alt := false;
	  n)
    else (alt := true;
	  ...
	  then (match i.Limit.incumbent with
	    | Limit.Nothing ->
	      (Limit.new_incumbent i (Limit.Incumbent (0.,n));
	       Dpq.clear openlist;
	       Dpq.clear helplist;
	       Htable.iter (fun _ n -> n.open_pos <- prev_it;
		 n.ha_pos <- prev_it) nodes;
	   ...
	   (* new itteration only begins on truly new incumbent*)
		...
		let help,norm = expand (List.nth wts !iteration) n in
		List.iter (consider_child true) help;
		List.iter (consider_child false) norm;
