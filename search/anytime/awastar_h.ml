...
let awastar ?(queue_record = Recorders.none) ?(get_win = (fun a -> a.h))
  ...
  Iterated_window_search.do_search
    ...
    i initial is_goal expand ordered_p better_p
...
let no_dups sif args =
  Search_args.is_empty "Awasta_h.no_dups" args;
  call_awastar Limit.results5 Limit.unwrap_sol5 sif


let dups sif args =
  Search_args.is_empty "Awasta_h.dups" args;
