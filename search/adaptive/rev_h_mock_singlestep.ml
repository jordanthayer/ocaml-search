...
let make_unbounded_correction get_g get_rh get_h get_d get_depth =
  (** Produces a corrected heuristic estimate.
  ...
  and f_calc n =
    let herr = Math.fmax (!h_err /. !num_samples) 0. in
      (get_g n) +. (get_h n) +. (herr *. (get_d n))
  in
    h_calc, f_calc


let make_clamped_correction get_g get_rh get_h get_d get_depth bound =
  ...
  and f_calc n =
    ...
    and herr = Math.fmax (!h_err /. !num_samples) 0. in
...
let make_clamped_fhp_correction ?(wt = None) get_g get_rh get_h get_d
    get_depth bound =
  (** Produces a corrected heuristic estimate which when used for a search
      will still return results which are within the [bound] desired.
      This correction uses backwards looking h error and depth to create
  ...
  and f_calc n =
    let wf = bound *. ((get_g n) +. (get_h n))
    and herr = Math.fmax (!h_err /. !num_samples) 0. in
    let fhat = (get_g n) +. ((get_h n) +. (herr *. (get_d n))) *. wt in
      Math.fmin wf fhat
  in
    h_calc, f_calc
