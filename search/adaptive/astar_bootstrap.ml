...
let dups_functions ?(feature_count = default_feature_count)
  ...
  and solver sface =
    let sol,_,_,_,_,_ =
      Best_first.search_dups sface f_then_g just_f setpos getpos in
      match sol with
	| Limit.Nothing -> None
...
let ida_dups_functions ?(feature_count = default_feature_count)
    ?(get_features = make_default_features) base_sface =
  (** Performs an A* search from the initial state to a goal,
      for domains where duplicates are frequently encountered. *)
  let learner = ann feature_count in
  ...
  and solver sface =
    let sol,_,_,_,_,_ =
      (Iterative_deepening_astar.reset_bound_default
	 ~quiet:true
	 sface.Search_interface.initial.f;
	Iterative_deepening.no_dups_in_dups_dom ~quiet:true
	 sface
	 just_f
	 Iterative_deepening_astar.see_expansion_default
	 (fun n -> if (n.f > !Iterative_deepening_astar.this_bound &&
			 n.f < !Iterative_deepening_astar.next)
	  then Iterative_deepening_astar.next := n.f;
	    n.f <= !Iterative_deepening_astar.this_bound)
	 (Iterative_deepening_astar.iteration_complete_default ~quiet:true)) in
      match sol with
	| Limit.Nothing -> None
	| Limit.Incumbent (q,n) -> Some (n, n.g)
  in make_sface, train, solver, i
...
let do_dups ~make_instance ~load_instance ~base_sface ~domain ~args =
  ...
  and ins_min = Search_args.get_int "Astar_bootstrap" args 0
  and tmin = Search_args.get_float "Astar_bootstrap" args 1
  and tmax = Search_args.get_float "Astar_bootstrap" args 2
...
let do_ida ~make_instance ~load_instance ~base_sface ~domain ~args =
  let iroot,attrs = Bootstrapping.domain_to_iroot_attrs domain
  and ins_min = Search_args.get_int "Ida_bootstrap" args 0
  and tmin = Search_args.get_float "Ida_bootstrap" args 1
  and tmax = Search_args.get_float "Ida_bootstrap" args 2
  and make_sface, train, solver, i = ida_dups_functions base_sface in
    (Bootstrapping.bootstrap ~ins_min ~tmax
       make_sface
       train
       solver
