(** Searches which learn a correction of fhat from single step error
    models.  Probably shouldn't be called directly.  Instead built upon
    by every model to cut down on the amount of clutter in the code. *)
...
let no_dups sface timer h_calc f_calc =
...
let dups sface timer h_calc f_calc =
...
let drop sface timer h_calc f_calc =
...
let anytime_no_dups sface timer h_calc f_calc =
  (** Performs a search in domains where there are no duplicates.
      [sface] is the domain's search interface
      [bound] is the desired quality bound
      [timer] is a timer from Timers.ml
...
let anytime_dups sface timer h_calc f_calc =
  (** Performs a search in domains where there are duplicates.
      [sface] is the domain's search interface
      [bound] is the desired quality bound
      [timer] is a timer from Timers.ml
  ...
  let search_interface = make_interface sface h_calc f_calc timer in
    Limit.unwrap_sol6 unwrap_sol
      (Continued_reorderable.search_dups
	 search_interface
	 est_f_then_d_then_g
	 better_p
	 setpos
