...
let update_model model get_t get_d get_f bound depth node children =
  (** [update_model model get_t get_d get_f bound depth node children]
      updates the model by looking at the given expansion. *)
  let pt = get_t node
  and pd = get_d node
    ...
    List.iter
	 ...
	 let delta_d = cd - pd in
	   ...
	   then	      (* check for inconsistent change in f *)
	     failwith (Wrutils.str "Inconsistent: pf=%f, cf=%f\n%!" pf cf);
...
let find_bound model seed_depth prev_fs seed_nodes desired =
  ...
  let bound = Hist.val_for_weight desired total_fs in
    ...
    *)
    ...
    bound, (Hist.weight_left_of bound total_fs)
