...
let no_model = make_no_smoothing 0 0
  (** A place-holder. *)
...
(**************************************************************)
(* Convenient functions for using the Iterative_model module. *)
(**************************************************************)

let count_model m starting_weights n =
  (** [count_model m starting_weights n] builds a count model function
      which takes a bound and returns the expected number of nodes
      expected at the given bound.  The difference between this
      function and the [e] and [e_wtd_avg] function is that this takes
      a float bound and returns an integer number of expected nodes.
      [n] is the function that estimates the number of nodes at a
      given depth in a search, [m] is the learned heuristic model and
      [starting_weights] are the weights to use for each type of
      starting position. *)
  let e_fun = e_wtd_avg m starting_weights n in
    (fun bound -> (Math.round (e_fun (Math.round bound))))
