(**
    Dynamically Weighted A* search algorithm as through the search interface
...
type 'a node = {
  ...
  depth : float;        (* Depth of node in tree.  Root @ 0 *)
...
let make_expand expand h wt goal_depth =
  ...
  (fun n ->
     List.map (fun (d, g) ->
		 ...
		 let fact = Math.fmax 1. (wt *. (1. -. dep /. goal_depth)) in
		   { data = d;
		     wf = g +. fact *. hv;
		     f = g +. hv;
		     g = g;
...
let no_dups sface args =
  ...
  let wt = Search_args.get_float "Dynamically_wted_astar.no_dups" args 0 in
...
let dups sface args =
  ...
  let wt = Search_args.get_float "Dynamically_wted_astar.dups" args 0 in
  let search_interface = Search_interface.make
    ~node_expand:(make_expand sface.Search_interface.domain_expand
		    sface.Search_interface.h wt
		    (sface.Search_interface.d sface.Search_interface.initial))
...
let drop_dups sface args =
  (** Performs a dynamically weighted A* search from the initial state to a
      goal for domains where duplicates are frequently encountered. *)
  let wt = Search_args.get_float "Dynamically_wted_astar.drop_dups" args 0 in
  let search_interface = Search_interface.make
    ~node_expand:(make_expand sface.Search_interface.domain_expand
		    sface.Search_interface.h wt
		    (sface.Search_interface.d sface.Search_interface.initial))
