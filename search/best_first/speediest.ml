...
let make_expand expand dist =
  ...
  (fun n ->
    ...
    let fdepth = float next_depth in
    List.map (fun (dat, g) ->
      let d = dist dat in
      { data = dat;
	d = d /. (d +. fdepth);
...
let make_expand_m3 expand dist =
  (** Takes the domain expand function and a heuristic calculator
      and creates an expand function which returns search nodes. *)
  (fun n ->
    let next_depth = n.depth + 1 in
...
let make_expand_m2 droot expand dist =
  (** Takes the domain expand function and a heuristic calculator
      and creates an expand function which returns search nodes. *)
  (fun n ->
    let next_depth = n.depth + 1 in
...
let no_dups sface args =
  ...
  Search_args.is_empty "Speedy.no_dups" args;
...
let dups_internal ?(silent = false) sface args =
  ...
  Search_args.is_empty "Speedy.dups" args;
  let search_interface =
    Search_interface.make
      ~node_expand:(make_expand sface.Search_interface.domain_expand
		      sface.Search_interface.d)
...
let dups_internal_m2 ?(silent = false) sface args =
  (** Performs a greedy search from the initial state to a goal,
      for domains where duplicates are frequently encountered. *)
  Search_args.is_empty "Speedy.dups" args;
  let search_interface =
    Search_interface.make
      ~node_expand:(make_expand_m2
...
let dups_m2 sface args =
    Limit.unwrap_sol6 unwrap_sol (dups_internal_m2 sface args)
...
let drop_dups_internal sface args =
  ...
  Search_args.is_empty "Speedy.drop_dups" args;
  let search_interface =
    Search_interface.make
      ~node_expand:(make_expand sface.Search_interface.domain_expand
		      sface.Search_interface.d)
...
let drop_dups_internal_m2 sface args =
  (** Performs a greedy search from the initial state to a goal,
      for domains where duplicates are frequently encountered.
      When duplicates are seen, they are ignored immediately. *)
  Search_args.is_empty "Speedy.drop_dups" args;
...
let drop_dups_internal_m3 sface args =
  (** Performs a greedy search from the initial state to a goal,
      for domains where duplicates are frequently encountered.
      When duplicates are seen, they are ignored immediately. *)
  Search_args.is_empty "Speedy.drop_dups" args;
...
let drop_dups_m2 sface args =
  Limit.unwrap_sol6 unwrap_sol (drop_dups_internal_m2 sface args)

let drop_dups_m3 sface args =
  Limit.unwrap_sol6 unwrap_sol (drop_dups_internal_m3 sface args)
