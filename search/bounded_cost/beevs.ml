...
let dhat_order a b =
  let av = a.vals
  and bv = b.vals in
  let aed = av.est_effort
  and bed = bv.est_effort in
    aed < bed || (aed = bed && av.est_d <= bv.est_d)
...
let make_expand i expand hd fd_calc cost_bound =
  ...
  (fun n ->
     ...
     let exp_delay = float (exp - n.generated) in
       List.fold_left
	 (fun (dac,fac) (s,g) ->
	    let h,d = hd s in
	    let f = g +. h in
...
let dups fd_calc sface args =
  ...
  let i = (Limit.make Limit.Nothing sface.Search_interface.halt_on better_p
		...
		(fun n -> truncate n.vals.depth))) in
  let root = make_root h d sface.Search_interface.initial
