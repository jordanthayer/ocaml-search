...
let dcheapest a b =
  let afp = a.fp
  and bfp = b.fp in
  let aed = afp.est_effort
  and bed = bfp.est_effort
  and aef = afp.est_d
  and bef = bfp.est_d in
  (aed < bed) ||
    ((aed = bed) && (aef <= bef))
...
let make_initial initial hd c =
  ...
  let fp = { h = h; d = d; h_err = 0.; d_err = 0.; g = 0.; f = h;
	     est_f = h; est_d = d; est_effort = d; potential = h /. c}
  and iv = {pos = np; depth = 0; generated = 0} in
...
let make_expand i expand hd cost_bound =
  let no_pos = Dpq.no_position in
  let rb = Ring_buffer.init 1000 in (* I should do a better study of the effect of this *)
  let expand n =
    let exp = i.Limit.expanded in
    let delay = float (exp - n.ints.generated) in
    let nfp = n.fp in
    let nd = n.ints.depth + 1
    and pf = nfp.f -. nfp.h_err
    and pd = nfp.d -. 1. -. nfp.d_err in
    let fnd = float nd in
    Ring_buffer.insert rb delay;
    let delay = Ring_buffer.get_mean rb in
	...
	and d_err = d -. pd (* becomes d -. n.d +. 1. +. nfp.d_err *) in
	 ...
	 let fp = { h = h; d = d; h_err = h_err; d_err = d_err; g = g; f = f;
		    ...
		    est_effort = est_d *. delay;
		    potential = h /. (cost_bound -. g)}
	 and ints = {pos = no_pos; depth = nd; generated = exp;} in
	 assert (est_d >= 0.);
	 assert (est_f >= f);
...
let make_expand_pm i expand hd cost_bound =
  let no_pos = Dpq.no_position in
  let expand n =
    let exp = i.Limit.expanded in
    let delay = float (exp - n.ints.generated) in
	...
	and d_err = d -. pd (* becomes d -. n.d +. 1. +. nfp.d_err *) in
	 ...
	 let est_f = g +. est_h in
	 let fp = { h = h; d = d; h_err = h_err; d_err = d_err; g = g; f = f;
		    est_f = est_f; est_d = est_d; est_effort = est_d *. delay;
		    potential = h /. (cost_bound -. g)}
	 and ints = {pos = no_pos; depth = nd; generated = exp;} in
	 assert (est_d >= 0.);
	 assert (est_f >= f);
...
let dups sface args =
  ...
  let root = make_initial sface.Search_interface.initial hd cost_bound
  and expand = (make_expand i sface.Search_interface.domain_expand
		  hd cost_bound)
...
let dups_pm sface args =
  ...
  let i = (Limit.make Limit.Nothing sface.Search_interface.halt_on better_p
		...
		(fun n -> n.ints.depth))) in
  let root = make_initial sface.Search_interface.initial hd cost_bound
  and expand = (make_expand_pm i sface.Search_interface.domain_expand
		  hd cost_bound)
  and goal_p = wrap sface.Search_interface.goal_p
  and key = wrap sface.Search_interface.key
  and hash = sface.Search_interface.hash
