c=======================================================================
     ...
     +  loopA, lret)
C lret added by Wheeler Ruml to provide return value
C      = 0 convergence and normal return
C      = -1 inconsistent bounds
C      = -2 no free variables, check input bounds
C      = -3 too many active variables in starting solution
C      = -4 too many free variables
C      = -5 failed to converge
c=======================================================================
...
C DEC f77 can handle the line with the min:    - Wheeler Ruml
C      dimension w(n), act(m,min(m,n)+2), zz(m), istate(n+1)
C Sun's f77 needs this line instead (note we assume n <= m!):   - Wheeler Ruml
      dimension w(n), act(m,n+2), zz(m), istate(n+1)
c
C Perhaps this could be smaller?  eg 1.0d-15? - Wheeler Ruml
      data eps/1.0d-11/
c
