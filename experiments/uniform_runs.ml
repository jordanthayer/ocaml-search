...
and solver_binary = ref (Experiments.bin_root ^ "uniform_tree_solver")
...
let do_batch
    overwrite ?(insts=All_instances) limits costs ~max_sol_br
    ~min_depth ~max_depth args alg_attrs =
  (** [do_batch overwrite limits costs max_sol_br min_depth max_depth
      args alg_attrs] performs a batch of runs on all matching
      instances. *)
  let costs_str =
  ...
  in
    run_batch overwrite insts limits args ~alg_attrs
      ~batch_attrs:[ "model", "uniform-tree";
		     "costs", costs_str;
