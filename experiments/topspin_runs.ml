(**
   ...
   For running topspin experiments
...
let instance_root =
  Filename.concat Experiments.instance_root "topspin"

let solver_binary =
  ref (Filename.concat Experiments.bin_root "topspin_solver")
  ...
  let cmd = Printf.sprintf "ulimit -s unlimited\n%s %s %s < %s" !solver_binary args limit_args prob_path in
	...
	(fun to_solver from_solver err_solver ->
	   let buf = Buffer.create 16 in
		 ...
		 res);
    in
      Notify.try_and_email_on_fail (run cmd) "Topspin_runs"
...
let do_run overwrite limits args attrs prob_path =
  ...
  let data_root = Filename.concat Experiments.data_root "topspin" in
...
let do_batch ?(send_mail=true) 
    ...
    ~disks ~flipper ~heuristic args alg_attrs =
  ...
  let batch_str = Printf.sprintf "disks=%d, flipper=%d heuristic %s" 
    disks flipper heuristic in
...
let do_basic_batch ?(overwrite = false)
    ...
    ~disks ~flipper ~heuristic
    alg =
  (* greedy, speedy, a_star *)
  let send_mail = false in
    Notify.start_batchtime();
    do_batch ~send_mail ~overwrite ~cost ~limits 
      ~flipper ~disks ~heuristic
      alg ["alg", alg];
    Notify.send_batch_completed_mail "Topspin_runs" "Basic" alg
  ...
  let send_mail = false in
    ...
    List.iter
	 ...
	 do_batch ~send_mail ~overwrite ~cost ~limits 
      ...
      weights;
    Notify.send_batch_completed_mail "Topspin_runs" "Weighted" alg
  ...
  let send_mail = false in
    ...
    List.iter
	 ...
	 do_batch ~send_mail ~overwrite ~cost ~limits 
      ...
      beam_widths;
    Notify.send_batch_completed_mail "Topspin_runs" "Old Beam" alg
...
let do_hbss_batch ?(overwrite = false)
  ...
  let beam_widths = List.map (fun a -> node_capacity / a) beam_widths in
	...
	beam_widths;) biases;
    Notify.send_batch_completed_mail "Topspin_runs" "Hbss Beam" alg
  ...
  let send_mail = false in
    ...
    List.iter
	 ...
	 let depth_bound_i = match depth_bound with
	     Static db -> db 
	   | Dynamic dp -> int_of_float (dp *. (float_of_int (calculate_bound
								disks flipper))) 
	 ...
	 let node_capacity = 
      ...
      beam_widths;
    Notify.send_batch_completed_mail "Topspin_runs" "BULB" alg
